...
- 랭킹: 경기·승리·패배·상대 수를 누적 집계하며 동점자는 같은 순위로 표시
...
배포 후 API의 Render Shell에서 `python -m app.cli create-admin --username admin`을 실행합니다. 이어서 `/health`, 관리자 로그인, 선수 등록 순서로 확인합니다.
