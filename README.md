...
- 일반 경기: 서울 날짜 기준으로 같은 두 선수는 하루에 한 경기만 등록 가능. 개인 리그·단체전 경기는 이 제한에서 제외
- 랭킹: 선수 계정만 대상으로 경기·승리·패배·상대 수를 누적 집계하며 동점자는 같은 순위로 표시
- 선수 기록: 랭킹의 선수를 누르면 선수 기준 경기 기록과 내 기준 맞대결 전적을 각각 최근 5건부터 조회하고, 더보기로 5건씩 확장. 승패 요약은 각 목록의 전체 기록 기준
- 정산: 각 부문 기록 10개당 추첨권 1장과 전체 중 본인의 비율 표시
- 추첨권 분포: 정산 부문을 누르면 상위 5명과 내 위치를 표시하고, 전체 보기는 보유 선수를 10명씩 확장. 막대는 최다 보유자 대비 추첨권 수, 확률은 전체 추첨권 기준이며 다음 1장까지의 진행도도 제공
- 동전 던지기: 한국 날짜 기준 하루 20회 시도. 20회가 남아 시작한 게임에서 아직 앞면·뒷면을 고르지 않았다면, 시작에 사용한 1회를 포함해 총 20회를 소진하고 연속 4회부터 이어갈 수 있음
- 탁구공 날리기: 무제한 도전과 최고 점수 랭킹. 무작위 보물상자에 닿으면 배경·탁구채·탁구공 스킨을 계정에 저장하며, 시작 버튼 아래에서 선택하면 미리보기에 바로 반영
- 스킨 수집: 기본 3종과 수집용 24종. 배경·탁구채·탁구공을 각각 3×3으로 선택하며, 일반·희귀·전설 등급으로 중복 획득할 수 있음. 공 크기·속도·충돌 판정은 동일하며 게임 도중에는 시작할 때 선택한 외형을 유지. 점수만 초기화할 때는 수집품을 보존
- 보물상자 빈도: 연속 등장 방지 2구간을 포함해 지속 플레이 중 평균 30점 진행당 1개 생성(추첨 가능 구간 확률 1/28). 무작위 높이의 상자에 직접 닿아야 획득하며, 30점마다 확정 지급하는 방식은 아님
- 시작 전 미리보기: 공만 별도 투명 캔버스에서 CSS로 움직이며 게임 실행·화면 밖·백그라운드에서는 멈춤. 시스템의 동작 줄이기 설정에서도 움직임을 생략
- 개인 리그: 4~6명이 한 번씩 맞붙는 풀리그. 전체 승리 수, 동률 선수 간 승패, 전체 경기에서 승리한 세트 수 순으로 순위를 계산
- 단체전: 4인 팀들이 한 번씩 맞붙는 풀리그. 팀 대결은 4단식으로 진행하고 2:2일 때 각 팀의 단식 패배자 2명이 복식으로 승부 결정. 순위는 팀 대결 승리 수, 동률 팀 간 승패, 전체 단식·복식에서 승리한 세트 수 순으로 계산

리그전과 단체전의 단식 결과는 일반 경기와 같은 랭킹·정산 기록에 포함됩니다. 복식은 개인 경기 통계에는 포함하지 않습니다. 대회 경기는 일반 경기의 동일 상대 하루 한 번 제한에서 제외되므로, 같은 날 일반 경기와 대회 경기를 각각 기록하거나 서로 다른 대회에서 같은 상대와 경기할 수 있습니다. 같은 대진의 결과 중복 제출과 단체전 한 대결 안에서의 선수 중복 출전은 계속 제한합니다.

동률 선수·팀 간 승패는 전체 승리 수가 같은 그룹 안에서의 승리 수를 비교하며, 세 기준까지 같으면 공동 순위입니다. 승리한 세트 수에는 패한 경기에서 얻은 세트도 포함됩니다. 단체전의 팀 승리·맞대결 승리는 완료된 팀 대결 기준이고, 세트 수는 진행 중인 팀 대결을 포함해 기록된 모든 단식·복식 점수를 합산합니다.

모든 대진의 결과가 등록되면 자동으로 완료 상태가 되고, 완료된 다음 날 한국 시간 0시부터 종료 상태가 됩니다. 완료된 대회는 진행 중 목록 안에서 파란색 배지로 표시하며, 종료된 대회는 회색으로 구분합니다. 진행 중 목록은 내 참가 여부를 먼저 보고, 그다음 최근 생성순으로 정렬합니다.

단체전 생성 시 팀 이름은 A, B, C…가 기본값이며 직접 수정할 수 있습니다. 팀을 추가·삭제해도 다른 팀에 입력한 이름은 유지됩니다. 대회명은 언제든 수정할 수 있습니다. 단체전의 팀 이름도 경기 시작 후 수정할 수 있지만, 참가 선수와 팀 편성은 첫 결과가 등록된 뒤 잠깁니다. 관리자는 완료·종료 후에도 결과를 바로잡을 수 있고, 결과 삭제나 복식 출전자 변경으로 대회가 미완료 상태가 되면 자동으로 진행 상태로 돌아갑니다. 대회를 삭제하면 연결된 단식·복식과 해당 단식이 반영했던 랭킹·정산 기록도 함께 제거되므로 화면에서 정확한 대회명을 다시 입력해야 합니다.
...
백엔드는 `backend/constraints.txt`, 프론트엔드는 `frontend/pnpm-lock.yaml`의 검증된 버전을 사용합니다. Docker와 CI도 같은 파일을 적용하므로 재배포만으로 의존성이 임의로 바뀌지 않습니다. `constraints.txt`는 `-r` 대신 위와 같이 `--constraint`로 사용합니다. 버전을 변경할 때는 전체 검사를 다시 수행합니다.
...
- 새 버전은 앱 실행 직후·백그라운드에서 복귀·네트워크 재연결 시 확인하고, 화면을 보고 있는 동안에는 1분 간격으로 확인합니다. 오프라인·설치 중·중복 요청은 건너뛰며, 준비된 새 버전만 안내합니다. 업데이트 알림을 닫아도 홈 하단의 **새 버전 업데이트 안내**에서 다시 열 수 있고, 앱 재실행·복귀 시에도 다시 나타납니다. 정기 확인만으로 닫은 알림을 다시 열지는 않습니다. 업데이트는 사용자가 적용 버튼을 눌렀을 때 진행하며, 캐시 정책·로그인 유지 기간은 기존과 같습니다. 이 확인 주기는 해당 버전을 한 번 적용한 뒤부터 사용됩니다.
...
- 점수 저장 큐는 앱이 열린 동안만 유지합니다. 탁구공 게임 점수 저장이 실패하면 같은 계정에서 **점수 다시 저장**으로 재시도할 수 있습니다. 로그인 계정이 바뀌면 대기 기록은 제거됩니다. OS가 앱을 종료했을 때의 영구 보관·자동 재전송은 제공하지 않습니다.
- 보물상자는 예외적으로 저장 대기 중인 획득 요청을 계정별 localStorage에 남깁니다. 같은 계정으로 게임 화면을 다시 열거나 **상자 보상 다시 저장**을 누르면 같은 요청 번호로 재시도하므로 재추첨·중복 지급을 막습니다. 브라우저 저장소를 사용할 수 없거나 지우면 아직 서버에 전달되지 않은 요청은 복구할 수 없습니다.
...
- PostgreSQL: `basic-256mb` 컴퓨팅과 1GB 저장공간. 실제 청구액은 Render의 Billing 화면에서 확인
...
일상 관리, 마지막 배포 확인, 초기화 전 백업과 장애 대응은 [운영 안내](backend/OPERATIONS.md)를 참고하세요. 과거 검증 산출물·데모 생성 스크립트는 로컬 `backend/.backups/final-cleanup-20260913.zip`에 보관한 뒤 정리했습니다. `/output`, `/tmp`, `.logs`, `.backups`는 Git과 Docker 배포 대상에서 제외하며 테스트와 마이그레이션 파일은 유지합니다.
