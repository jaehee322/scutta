...
대회명은 언제든 수정할 수 있습니다. 단체전의 팀 이름도 경기 시작 후 수정할 수 있지만, 참가 선수와 팀 편성은 첫 결과가 등록된 뒤 잠깁니다. 관리자는 완료·종료 후에도 결과를 바로잡을 수 있고, 결과 삭제나 복식 출전자 변경으로 대회가 미완료 상태가 되면 자동으로 진행 상태로 돌아갑니다. 대회를 삭제하면 연결된 단식·복식과 해당 단식이 반영했던 랭킹·정산 기록도 함께 제거되므로 화면에서 정확한 대회명을 다시 입력해야 합니다.
...
- 점수 저장 큐는 앱이 열린 동안만 유지합니다. OS가 앱을 종료했을 때의 영구 보관·자동 재전송은 제공하지 않습니다.
