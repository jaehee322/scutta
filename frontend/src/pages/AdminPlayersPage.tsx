  ...
  return (
    <Modal title={title} description="이름은 로그인 아이디로 사용돼요." onClose={onClose}>
      <form className="modal-form" onSubmit={handleSubmit}>
        ...
        <div className="form-row"><label className="field"><span>성별</span><select value={form.gender} onChange={(event) => setForm({ ...form, gender: event.target.value as Gender })}><option value="M">남</option><option value="F">여</option></select></label><label className="field"><span>부수</span><input type="number" min={1} step={1} value={form.club_rank} onChange={(event) => setForm({ ...form, club_rank: event.target.value })} required /></label></div>
